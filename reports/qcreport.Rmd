...
if (sum(grepl("^Median$",rownames(raw_edata))) > 0) {
  ...
  s_metrics <- raw_edata[,s_selection]
}

orig_s_metrics <- s_metrics

mylog(rownames(s_metrics))
...
g_id_protein <- gg_points(id_protein, 
                          ...
                          col_string="analyticalBasename", label_string="`Analytical sample`",min_y=0) 
p_id_protein <- ggplotly(g_id_protein, tooltip=c("label")) %>% layout(annotations=protein_title) %>% layout(annotations=as_title)
...
g_cum_id_protein <- gg_points(cum_id_protein, 
                          y_string="ProteinCounts", x_string="analyticalBasename",
                          col_string="analyticalBasename", label_string="`Analytical sample`",min_y=0) 
p_cum_id_protein <- ggplotly(g_cum_id_protein, tooltip=c("label")) %>% layout(annotations=protein_title) %>% layout(annotations=as_title)
...
g_all_id_protein <- gg_points(all_id_protein, 
                          y_string="ProteinCounts", x_string="analyticalBasename",
                          col_string="analyticalBasename", label_string="`Analytical sample`",min_y=0) 
p_all_id_protein <- ggplotly(g_all_id_protein, tooltip=c("label")) %>% layout(annotations=protein_title) %>% layout(annotations=as_title)
...
>>>>>>> bbfebcacb04e8d8a6b22f8798833c75a1db6252a
