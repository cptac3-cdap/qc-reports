...
if (sort(unique(expdes$spectrafile)) != sort(unique(QCmetrics$spectrumBasename))) {                                     
  mylog(expdes)                                                                                                         
  mylog(QCmetrics)                                                                                                      
  stopifnot(sort(unique(expdes$spectrafile)) != sort(unique(QCmetrics$spectrumBasename)))                               
}                                                                                                                       
...
id_protein$analyticalBasenameFactor <- factor(id_protein$analyticalBasenameLabel,levels=unique(id_protein$analyticalBasename),ordered=TRUE)
...
iso_color = plotColorPalettes(samples_per_analyticalsample)
```
	              ...
	              sparse_x_labels=0)
{
  ...
  break.points=sort(as.numeric(unique(data[, x_string])))
...
# Check!
if (params$dochecks) {
  mylog(c(nFiles,nAnalyticalSamples,nFractionsPerAS,nRepsPerFrac))
...
if (nRepsPerFrac == 1) {
    ...
    mat_t[,3] <- as.matrix(t1)[,1]
    colnames(mat_t) <- c("Index","Analytical sample", "Fractions")
} else {
    mat_t <- matrix(NA, ncol=5, nrow=length(levels(QCmetrics$analyticalBasenameFactor)))
    mat_t[,1] <- sprintf("%02d",1:length(levels(QCmetrics$analyticalBasenameFactor)))
    mat_t[,2] <- levels(QCmetrics$analyticalBasenameFactor)
    mat_t[,3] <- as.matrix(t1)[,1]
    mat_t[,4] <- as.matrix(maxreps)[,1]
    for (i in 1:range(length(maxreps))) {
        if (minreps[i] != maxreps[i]) {
            mat_t[i,4] = sprintf("%s-%s",minreps[i],maxreps[i])
        }
    }
    mat_t[,5] <- as.matrix(t)[,1]
    colnames(mat_t) <- c("Index","Analytical sample", "Fractions","Replicates","Files")
}
...
g_cum_id_protein <- gg_points(cum_id_protein, 
                          y_string="ProteinCounts", x_string="analyticalBasename",
                          col_string="analyticalBasename", label_string="`Analytical sample`",label2_string="ProteinCounts",min_y=0) 
...
g_DP_byF <- gg_points_frac(QCmetrics,
                               ...
                               smooth=ifelse(nFractionsPerAS>3,TRUE,FALSE)) + 
...
```
```{r, child = if (protocol != 'Label-free') 'labels.Rmd' else 'empty.Rmd'} 
```
