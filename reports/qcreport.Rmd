...
# Generate the Plot 
test_batch <- as.data.frame(t(ref_metrics))
...
p<-ggplot(df_out,aes(x=PC1,y=PC2,color=`Analytical sample` ))
p<-p+geom_point()

p_pca_b <- ggplotly(p, tooltip=c("colour")) %>% layout(annotations = pca_title, xaxis = x_per, yaxis = y_per, showlegend=FALSE)
...
# Generate the plot
p2<-ggplot(df_out,aes(x=PC1,y=PC2,color=`Isobaric Label`))
p2<-p2+geom_point()+scale_colour_manual(values=iso_color)
p_pca_b2 <- ggplotly(p2, tooltip=c("colour")) %>% layout(annotations = pca_title, xaxis = x_per, yaxis = y_per, showlegend=FALSE , legend=list(orientation = "h", x = 0.18, y = -0.15, xanchor = "left", yanchor = "bottom"))
